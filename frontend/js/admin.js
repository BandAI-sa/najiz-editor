...
import { getModelBadgeTheme } from "./utils/model-badges.js";
...
function buildTag(label, options = {}) {
  const { tone = "", extraClass = "", theme = null, dir = "", dataAttributes = {} } = options;
  const toneClass = tone ? ` ${tone}` : "";
  const customClass = extraClass ? ` ${extraClass}` : "";
  const style = theme ? ` style="--admin-tag-bg: ${theme.bg}; --admin-tag-text: ${theme.text};"` : "";
  const direction = dir ? ` dir="${dir}"` : "";
  const attributes = Object.entries(dataAttributes)
    .map(([key, value]) => ` data-${key}="${escapeHtml(String(value))}"`)
    .join("");

  return `<span class="admin-tag${toneClass}${customClass}"${style}${direction}${attributes}>${escapeHtml(label)}</span>`;
}

function buildModelBadge(model) {
  const badge = getModelBadgeTheme(model);
  return buildTag(badge.label, {
    extraClass: "admin-tag-model",
    theme: badge.colors,
    dir: "ltr",
    dataAttributes: { "model-key": badge.key },
  });
}

function buildGeneratedByRow(model, extraClass = "") {
  const className = extraClass ? `admin-model-row ${extraClass}` : "admin-model-row";
  return `
    <div class="${className}">
      <span class="admin-model-row-label">Generated by</span>
      ${buildModelBadge(model)}
    </div>
  `;
}
  ...
  elements.list.innerHTML = state.items
    .map((item) => {
      const tags = [
        ...
        item.review_score !== null ? buildTag(`التقييم ${item.review_score}%`, { tone: "success" }) : "",
        item.issue_count ? buildTag(`${item.issue_count} ملاحظات`, { tone: "warning" }) : "",
      ]
        .filter(Boolean)
        .join("");
      ...
      return `
        <button type="button" class="admin-record${activeClass}" data-petition-id="${escapeHtml(item.petition_id)}">
          ...
          ${buildGeneratedByRow(item.model, "admin-record-model")}
  ...
  elements.detail.innerHTML = `
    <div class="admin-detail-header">
      <div>
        ...
        ${buildGeneratedByRow(petition.model, "admin-detail-model")}
      </div>
